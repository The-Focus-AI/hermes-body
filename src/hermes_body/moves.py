...
class BreathingMove(Move):
    ...
    def evaluate(self, t: float) -> tuple:
        ...
        if t < self.interpolation_duration:
            ...
            z_offset = self.breathing_z_amplitude * np.sin(2 * np.pi * self.breathing_frequency * breathing_t)
            head_pose = create_head_pose(x=0, y=0, z=z_offset, roll=0, pitch=0, yaw=0, degrees=True, mm=False)

            antenna_sway = self.antenna_sway_amplitude * np.sin(2 * np.pi * self.antenna_frequency * breathing_t)
...
class HeadLookMove(Move):
    ...
    def update_start(self, pose: NDArray[np.float32], antennas: tuple[float, float]) -> None:
        """Recalibrate the start pose — called by MovementManager when the move
        actually starts executing, so chained moves don't snap back."""
...
class MovementManager:
    ...
    def _update_thinking_offsets(self, current_time: float) -> None:
        ...
        if self._processing:
            ...
            self._thinking_amplitude = max(0.0, self._thinking_amplitude - 2.0 * self.target_period)
    ...
    def _manage_move_queue(self, current_time: float) -> None:
        ...
        if self.state.current_move is None and self.move_queue:
            ...
            # Recalibrate start pose so chained moves don't snap back
            try:
                current_head = self.current_robot.get_current_head_pose()
                _, current_ant = self.current_robot.get_current_joint_positions()
                if hasattr(self.state.current_move, "update_start"):
                    self.state.current_move.update_start(current_head, tuple(current_ant))
            except Exception:
                pass
    ...
    def _get_secondary_pose(self) -> FullBodyPose:
        ...
        offsets = [
            self.state.speech_offsets[i] + self.state.face_tracking_offsets[i] + self.state.thinking_offsets[i]
            for i in range(6)
        ]
